...
export class XmlFormatter extends BaseFormatter {
  ...
  async formatOutput(
    ...
    for (const { absPath, relPath } of files) {
      ...
      parts.push(`  <file path="${escapeXml(normPath)}" language="${escapeXml(lang)}"><![CDATA[${safeContent}]]></file>`)
    }
  ...
  async writeOutput(
    ...
    for (const { absPath, relPath } of files) {
      ...
      const normPath = relPath.replace(/\\/g, '/')
      const safeContent = content.replace(/]]>/g, ']]]]><![CDATA[>')
      const line = `  <file path="${escapeXml(normPath)}" language="${escapeXml(lang)}"><![CDATA[${safeContent}]]></file>\n`
